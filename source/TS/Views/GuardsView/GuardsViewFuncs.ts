...
export async function displayGuardData(items: any,
    tableBody: UIElement,
    rowsPerPage: number,
    page: number,
    ...
    for(index = 0; index < arrayGuards.length; index++) {
        ...
        states?.forEach((state: UIElement) => {
            if (state.innerText === "Enabled") state.classList.add("user_active"), state.innerText = "Activo"
            else if (state.innerText === "Disabled") state.classList.add("user_inactive"), state.innerText = "Inactivo"
        })
    }
